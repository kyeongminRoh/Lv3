...
//        const { categoryId } = req.params;
        ...
        let order = 1;
        const orderCount = await prisma.Categories.findFirst({ // 데이터베이스에서 첫번째 카테고리를 가져오기
...
router.get('/categories', async (req, res, next) => {
    const categories = await prisma.Categories.findMany({ // findManyDB 레코드 목록들을 불러오기
        select: { // 반환될 개체에 포함할 속성을 지정한다.
            categoryId: true,
            order: true,
            name: true
        }
...
router.delete('/categories/:categoryId', async(req, res, next) => {
    ...
    if (!categories) {
        return res.status(404).json({ errorMessage: "카테고리가 존재하지 않습니다."})
    }
